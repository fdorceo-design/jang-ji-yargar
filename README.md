...
2. 配置直後から、ターン担当者自身の通常セグメントが始まる。ただし配置したユニットだけは、この最初のセグメントに限り「移動不可・挑戦は配置位置から最大1回まで」の制限を受ける（相手の通常セグメントを1回経て自分に戻ってきた時点で解除）。他のユニットは通常どおり行動できる。
...
- 盤面・UI：プレイ可能なプロトタイプあり（[index.html](./index.html)）
- プレイテスト：進行中
